...
SSD::SimVector<long> GetNavigateRoadIdList(const SSD::SimPoint3D &startPt,
  ...
  SimOneAPI::Navigate(ptList, indexOfValidPoints,
                      naviRoadIdList); // 获取规划路径所途径道路的ID列表
...
bool GetValidSuccessor(const HDMapStandalone::MLaneId &laneId,
  ...
  bool valid =
      SimOneAPI::GetLaneLink(laneId.ToString(), laneLink); // 获取临近车道信息
  assert(valid);
  ...
  for (auto &successorLane : laneLink.successorLaneNameList) {
    ...
    } else // 如果有下一个section就返回true，如果没有就返回false
    {
      ...
      if (successorId.sectionIndex ==
          laneId.sectionIndex + 1) // 这个递归函数不知道有没有出错？？
      {
...
void AddSamples(const HDMapStandalone::MLaneId &laneId,
  ...
  if (SimOneAPI::GetLaneSample(laneId.ToString(), laneInfo)) {
    path.reserve(path.size() +
                 laneInfo.centerLine.size()); // 给path容器分配空间
    for (auto &pt : laneInfo.centerLine) {
      path.push_back(pt);
    }
    // std::cout << " findpoint: " << path.back().x << ", " << path.back().y <<
    // std::endl; std::cout << " laneid: " << laneId.roadId << ", " <<
    // laneId.sectionIndex << ", " << laneId.laneId << std::endl;
  }
}
...
std::vector<obstaclestruct> GetObstacleList() {
  ...
  /*SSD::SimPoint3D pt;
  SSD::SimString ownerLaneId;*/
  std::unique_ptr<SimOne_Data_Obstacle> pSimOne_Data_Obstacle =
      std::make_unique<SimOne_Data_Obstacle>();
  if (SimOneAPI::GetGroundTruth(0, pSimOne_Data_Obstacle.get())) {
    // cout << SimOneSM::GetGroundTruth(0, pSimOne_Data_Obstacle.get()) << "
    // size: " << pSimOne_Data_Obstacle->obstacleSize << endl; std::cout
    // <<"obstacleSize+++++++++++++++++++++"<<
    // pSimOne_Data_Obstacle->obstacleSize << std::endl;
    for (int i = 0; i < pSimOne_Data_Obstacle->obstacleSize; i++) {
      ...
      double speed =
          UtilMath::calculateSpeed(pSimOne_Data_Obstacle->obstacle[i].velX,
                                   pSimOne_Data_Obstacle->obstacle[i].velY,
                                   pSimOne_Data_Obstacle->obstacle[i].velZ);
      allObstacle.push_back(obstaclestruct{SSD::SimPoint3D(posX, posY, posZ),
                                           laneId, type, speed, id});
    }
  }
...
int GetNearObstacleIndex(const SSD::SimPoint3D &mainVehiclepos,
                         ...
                         const vector<obstaclestruct> &Obstaclelist,
  ...
  for (unsigned int i = 0; i < Obstaclelist.size(); i++) {
    ...
    if (GetS(mainVehiclepos, mainVehicleLane) >
        GetS(
            obstacle.pt,
            mainVehicleLane)) // 如果最近的障碍物出现在主车的后方：先判断速度，再判断距离
    {
...
bool GetdetectingZone(const SSD::SimString &currentlaneId,
  ...
  double distance =
      UtilMath::distance(mainVehiclepos, Obstacle.pt); // 主车到障碍物的距离

  SimOneAPI::GetLaneMiddlePoint(
      Obstacle.pt, turntolane, changeToPoint,
      dir); // 获得障碍物相对于左侧车道的投影点API::GetLaneST(turntolane,
            // changeToPoint, s, t);//获得投影点的st坐标
  t_left = t - 1.7;
  t_right = t + 1.7;
  s_front = s + 10;
  s_back = s - distance - 7;
...
// 这个函数是检测变道区域里是否有物体
bool IsChangeLaneOccupied(const vector<obstaclestruct> &Obstaclelist,
                          SSD::SimPoint3DVector &detectingZone) {
...
SSD::SimPoint3DVector ChangeLanePathWithPoint(const SSD::SimPoint3D &from,
  ...
  ChangeLanePath(int(UtilMath::distance(from, to)), from, PointNextFrom, to,
                 PointNextTo, path);
  return path;
}
...
void GetChangeLanePath(const double &vel, const obstaclestruct &Obstacle,
  ...
  {
    // std::cout << "changeToPoint x:" << changeToPoint.x << "changeToPoint y:"
    // << changeToPoint.y << "z:" << changeToPoint.z << std::endl;
  }
...
bool IsGreenLight(const long &lightId, const SSD::SimString &laneId,
                                 ...
                                 &trafficLight)) // 得到仿真场景中的交通灯的真值
  {
    ...
    // 如果是绿灯，看灯的count是否小于10
      ...
      // std::cout << "trafficLight.countDown: " << trafficLight.countDown <<
      // std::endl;//倒计时
...
SSD::SimString GetTargetSuccessorLane(
  ...
  for (auto &successorLane : successorLaneNameList) {
    HDMapStandalone::MLaneId id(
        successorLane); // 将string格式的id转变成可提取的MLaneId模式
    for (auto &validitie : validities) {
      if ((id.roadId == validitie.roadId &&
           id.sectionIndex == validitie.sectionIndex &&
           id.sectionIndex == validitie.fromLaneId) ||
          (id.roadId == validitie.roadId &&
           id.sectionIndex == validitie.sectionIndex &&
...
bool Passed(const SSD::SimPoint3D &vehiclePos, const SSD::SimPoint3D &light,
  ...
  double s, t;
  bool found =
      SimOneAPI::GetLaneST(laneId, light, s, t); // 得到障碍物在车道上的st坐标
  double sveh, tveh;
...
SSD::SimPoint3DVector
GetdetectZone_realtime(const unique_ptr<SimOne_Data_Gps> &gps,
...
SSD::SimPoint3DVector GetdetectZone_fixed(const SSD::SimPoint3D &pos,
...
// 获取与前车的s距离
double GetSdistance(const unique_ptr<SimOne_Data_Gps> &gps,
                    const obstaclestruct &obstacle,
  ...
  if (id_car.roadId <= id_obst.roadId) {
    ...
    while (current_id.roadId <= id_obst.roadId) {
      ...
      double s_temp;
      SimOneAPI::GetRoadST(current_id.ToString(), centerline.back(), s_temp, t,
                           z);
      s_distance += s_temp;
...
bool CrosswalkOccupied(
    const SSD::SimPoint3DVector &crosswalkconst,
  ...
  for (int i = 0; i < pSimOne_Data_Obstacle->obstacleSize; i++) {
    ...
    xfront = obs.posX + obs.width / 2;
    xbehind = obs.posX - obs.width / 2;
    yfront = obs.posY + obs.width / 2;
    ybehind = obs.posY - obs.width / 2;
    ...
    // 将方框加入到zone中
    if ((obs.type == 6 || obs.type == 4) && CalculateObstacleSpeed(obs) != 0) {
      for (auto &pt : Zone) {
        // 判断边角是否在车道上
        if (IsOccupied(pt, crosswalkconst)) {
          return true;
        }
      }
    }
...
// 检测路口是否有障碍物
bool DetectJunctionObstacle(const unique_ptr<SimOne_Data_Gps> &gps,
  ...
  if (DetectJunction(gps)) {
    ...
    /*cout << " 0: " << junction_detectZone[0].x << " , " <<
    junction_detectZone[0].y << endl; cout << " 1: " << junction_detectZone[1].x
    << " , " << junction_detectZone[1].y << endl; cout << " 2: " <<
    junction_detectZone[2].x << " , " << junction_detectZone[2].y << endl; cout
    << " 3: " << junction_detectZone[3].x << " , " << junction_detectZone[3].y
    << endl;*/
...
// 检测最近的标志
bool DetectClosestSign(const unique_ptr<SimOne_Data_Gps> &gps,
  ...
  for (int i = 0; i < TrafficSignList.size();
       i++) // 通过遍历，找出当前离得最近的交通标志
  {

    auto &sign = TrafficSignList[i];
    double distanceSign = UtilMath::distance(
        mainVehiclepos, SSD::SimPoint3D(sign.pt.x, sign.pt.y, sign.pt.z));
    if (distanceSign < minDist) {
      minDist = distanceSign;
      closestSignIndex = i;
    }
  ...
  if (closestSignIndex == -1) {
    //std::cout << "TrafficSignListSize==0" << endl;
    return false;
  }
...
// 检测车辆半径内的的限速标志
bool DetectSpeedLimitSignInR(const SimOne_Data_Gps &gps, int &warningSignIndex,
                             double r, double &limit_speed) {
  SSD::SimVector<HDMapStandalone::MSignal> TrafficSignList;
  SimOneAPI::GetTrafficSignList(TrafficSignList);
...
bool DetectNearstSpeedLimitSign(const unique_ptr<SimOne_Data_Gps> &gps,
                                double &limitspeed) {
  SSD::SimVector<HDMapStandalone::MSignal> TrafficSignList;
  SimOneAPI::GetTrafficSignList(TrafficSignList);
  ...
  if (DetectClosestSign(gps, closestSignIndex,
                        currentdistance)) // 获取最近的交通标志
  {
    // cout << "i an in" << endl;
    auto &targetSign = TrafficSignList[closestSignIndex];
    TrafficSignType sign =
        GetTrafficSignType(targetSign.type); // 确定交通标志的type
    ...
    if (sign == TrafficSignType::SpeedLimit_Sign) {
      ...
      std::string speedLimitValue =
          targetSign.value.GetString(); // 获得限速值（字符串格式）
      double distanceSign = distance;
      double speedLimit;
      std::stringstream ss; // 把字符型转成浮点型
      ss << speedLimitValue;
      ss >> speedLimit;
      // cout << "value: "<<speedLimitValue <<" speedlimit: "<<speedLimit <<
      // endl;
...
bool DetectObstacleAhead_zoneVersion(const unique_ptr<SimOne_Data_Gps> &gps,
  ...
  SSD::SimPoint3DVector detectZone =
      GetdetectZone_realtime(gps, 25, 0, -1.5, 1.5);
  ...
  currentdistance = UtilMath::distance(
      MainVehiclePos,
      SSD::SimPoint3D(obstacle.pt.x, obstacle.pt.y, obstacle.pt.z));
...
// 得到一个同车道前方最近的障碍物
bool DetectObstacleAhead(const unique_ptr<SimOne_Data_Gps> &gps,
                         obstaclestruct &obstacle,
                         const SSD::SimVector<long> &roadidlist,
                         double &currentdistance) {
  int obstacleClosestIndex = -1;
...
HDMapStandalone::MSignal
GetTargetLight(const HDMapStandalone::MLaneId &id, const SSD::SimString &laneId,
               const SSD::SimVector<long> &roadIdList) {
  ...
  for (auto &item : lightList) {
    ...
    for (auto &ptValidities : item.validities) {
      auto it =
          std::find(roadIdList.begin(), roadIdList.end(), ptValidities.roadId);
...
SSD::SimPoint3D GetTragetStopLine(const HDMapStandalone::MSignal &light,
                                  const SSD::SimString &laneId) {
  ...
  if (stoplineList.size() == 0) {
    //cout << "stoplineList.pt==Max" << endl;
    return SSD::SimPoint3D(std::numeric_limits<double>::max(),
...
// 检测没有信号的停车线
bool DetectNolightStopline(const unique_ptr<SimOne_Data_Gps> &gps,
                           SSD::SimPoint3D &stopline,
                           SSD::SimPoint3DVector &crosswalk,
                           double &currentdistance) {
  SSD::SimVector<HDMapStandalone::MSignal> TrafficSignList;
  SimOneAPI::GetTrafficSignList(TrafficSignList);
  SSD::SimPoint3D mainVehiclepos(gps->posX, gps->posY, gps->posZ);
                        ...
                        signdistance)) // 获取最近的交通标志
  {
    ...
    {
      // cout << "i am in" << endl;
      stopline = {TrafficSignList[closestSignIndex].pt.x + 3.1,
                  TrafficSignList[closestSignIndex].pt.y + 0.7, 0};
      ...
      knot = {TrafficSignList[closestSignIndex].pt.x + 8.1,
              TrafficSignList[closestSignIndex].pt.y + 15.1, 0};
      crosswalk.push_back(knot);
      knot = {TrafficSignList[closestSignIndex].pt.x + 4.1,
              TrafficSignList[closestSignIndex].pt.y + 15.1, 0};
      crosswalk.push_back(knot);
      knot = {TrafficSignList[closestSignIndex].pt.x + 4.1,
              TrafficSignList[closestSignIndex].pt.y - 3.2, 0};
...
// 检测路口是否拥堵（得到下下车道的id）
bool IsJunctionCrowed(const unique_ptr<SimOne_Data_Gps> &gps,
                      const SSD::SimVector<long> &roadidlist) {
  SSD::SimPoint3DVector centerline;
  SSD::SimPoint3D mainVehiclepos(gps->posX, gps->posY, gps->posZ);
  SSD::SimString laneid = GetNearMostLane(mainVehiclepos);
  ...
  if (DetectJunction(gps)) {
    int index = find(roadidlist.begin(), roadidlist.end(), id.roadId) -
                roadidlist.begin(); // 找到当前道路的index
    GetValidSuccessor(id, roadidlist[index], roadidlist[index + 1], next_id);
    GetValidSuccessor(next_id, roadidlist[index + 1], roadidlist[index + 2],
                      towards_id); // 得到对面车道的id

    centerline = GetLaneSample(towards_id.ToString()); // 得到中心线

    // 生成25米的检测区域
    SSD::SimPoint3D startline =
        centerline.front(); // 车道中心线最后一点为停止线
    SSD::SimPoint3DVector crowed_detectZone;
    double s, t, s_front, s_back, t_left, t_right;
    SSD::SimPoint3D Zone, dir2;

    SimOneAPI::GetLaneST(towards_id.ToString(), startline, s,
                         t); // 获得投影点的st坐标
    t_left = t - 1.7;
    t_right = t + 1.8;
    s_front = s + 25;
    ...
    // 将检测区域的四个点从st坐标转换回xyz坐标，并放入检测区域点集
    SimOneAPI::GetInertialFromLaneST(towards_id.ToString(), s_front, t_right,
                                     Zone, dir2);
    crowed_detectZone.push_back(Zone);
    SimOneAPI::GetInertialFromLaneST(towards_id.ToString(), s_front, t_left,
                                     Zone, dir2);
    crowed_detectZone.push_back(Zone);
    SimOneAPI::GetInertialFromLaneST(towards_id.ToString(), s_back, t_left,
                                     Zone, dir2);
    crowed_detectZone.push_back(Zone);
    SimOneAPI::GetInertialFromLaneST(towards_id.ToString(), s_back, t_right,
                                     Zone, dir2);
    crowed_detectZone.push_back(Zone);

    cout << " 0: " << crowed_detectZone[0].x << " , " << crowed_detectZone[0].y
         << endl;
    cout << " 1: " << crowed_detectZone[1].x << " , " << crowed_detectZone[1].y
         << endl;
    cout << " 2: " << crowed_detectZone[2].x << " , " << crowed_detectZone[2].y
         << endl;
    cout << " 3: " << crowed_detectZone[3].x << " , " << crowed_detectZone[3].y
         << endl;
...
// 检测是否可以变道
bool DetectIsTurnable(const unique_ptr<SimOne_Data_Gps> &gps,
  ...
  if (lanelink.leftNeighborLaneName.Empty() == 0 &&
    ...
    detectzone = GetdetectZone_fixed(changeToPoint, s_front, s_back, -1.75,
                                     1.75); // 左车道检测区
...
bool IsInChangeLane(const SSD::SimPoint3DVector &targetPath,
                    SimOne_Data_Gps *pGps, HDMapStandalone::MLaneLink &lanelink,
                    string &turnlight) {
  SSD::SimString lanenow, laneforward;
  std::vector<float> pts;
  for (size_t i = 0; i < targetPath.size(); ++i) {
    pts.push_back(pow((pGps->posX - (float)targetPath[i].x), 2) +
                  pow((pGps->posY - (float)targetPath[i].y), 2));
  }
  ...
  float progTime = 0.8f;
  float mainVehicleSpeed =
      sqrtf(pGps->velX * pGps->velX + pGps->velY * pGps->velY +
            pGps->velZ * pGps->velZ);
  float progDist = mainVehicleSpeed * progTime > minProgDist
                       ? mainVehicleSpeed * progTime
                       : minProgDist;

  for (; index < targetPath.size(); ++index) {
    forwardIndex = index;
    float distance = sqrtf(((float)pow(targetPath[index].x - pGps->posX, 2) +
                            pow((float)targetPath[index].y - pGps->posY, 2)));
  ...
  lanenow =
      GetNearMostLane(SSD::SimPoint3D(pGps->posX, pGps->posY, pGps->posZ));
...
ChangeLanePathWithLane(const SSD::SimPoint3D &mainVehiclepos, double speed,
  ...
  SimOneAPI::GetInertialFromLaneST(currentlaneid, s_changePath, t, foward_point,
                                   dir);
...
// 障碍物变道函数
SSD::SimPoint3DVector ChangeLanePathWithObstacle(
    const SSD::SimPoint3D &mainVehiclepos, const SSD::SimString &changetolane,
    SSD::SimPoint3D &obstaclepos, SSD::SimPoint3D &TurnToPoint) {
  SSD::SimPoint3DVector changelane_path;
  SSD::SimPoint3D dir;
...
void PrintTargetPath(SSD::SimPoint3DVector &targetpath) {
  ...
  for (int i = 0; i < targetpath.size(); i += 1) {
    std::cout << "Point[" << i << "]==  " << targetpath[i].x << ","
              << targetpath[i].y << "," << targetpath[i].z << endl;
  }
}
...
int main() {
  ...
  std::unique_ptr<SimOne_Data_Obstacle> obstaclesPtr =
      std::make_unique<SimOne_Data_Obstacle>();
  std::unique_ptr<SimOne_Data_Signal_Lights> SignalLightsPtr =
      std::make_unique<SimOne_Data_Signal_Lights>(); // 信号灯
  ...
  /* 实现评价服务初始化 */
  SimOneAPI::InitEvaluationServiceWithLocalData(
      MainVehicleId); // 实现评价服务初始化
  ...
  /* 初始化 */
  SignalLightsPtr->signalLights = 0; // 一开始先设置不打任何信号灯
  ...
  /* 获得规划路径的道路列表 */
  SSD::SimVector<long> naviRoadIdList =
      GetNavigateRoadIdList(startPt, endPt); // 获得规划路径的道路列表
  ...
  while (true) {
    ...
    SSD::SimVector<int> indexOfValidPoints;
    mainVehiclePos = {gpsPtr->posX, gpsPtr->posY,
                      gpsPtr->posZ}; // 获得主车的位置
    ...
    double mainVehicleSpeed = UtilMath::calculateSpeed(
        gpsPtr->velX, gpsPtr->velY, gpsPtr->velZ); // 获得主车总速度(m/s)
    double mainVehicleAccel = UtilMath::calculateSpeed(
        gpsPtr->accelX, gpsPtr->accelY, gpsPtr->accelZ); // 获得主车的实时加速度
    ...
    HDMapStandalone::MSignal light;
    SSD::SimPoint3D stopline = GetTragetStopLine(
        GetTargetLight(HDMapStandalone::MLaneId(currentLaneID), currentLaneID,
                       naviRoadIdList),
        currentLaneID);
    ...
    DetectStopline(mainVehiclePos, naviRoadIdList, light_stopline, light,
                   crosswalk,
                   light_stopline_distance); // 获得最近的交通灯关联停车线
    DetectNolightStopline(
        gpsPtr, nolight_stopline, crosswalknots,
        nolight_stopline_distance); // 获取最近的无交通灯停车线
    std::cout << nolight_stopline.x;
    std::cout << nolight_stopline.y;
    std::cout << nolight_stopline.z;
    DetectJunctionObstacle(gpsPtr, junction_stopline,
                           junction_o_distance); // 获取路口处的障碍物
    // 获取同车道前方障碍物
    bool ObsExist =
        DetectObstacleAhead(gpsPtr, Obstacle_ahead, naviRoadIdList,
                            obstacleAhead_distance); // 获取同车道的前方障碍物
    ...
    // 检测是否在路口
    if (light_stopline_distance < 40 || (nolight_stopline_distance < 40) ||
        StopLineDistance <= 40) {
      std::cout << "in NearIntersection" << endl;
      RunState = "NearIntersection";
      isNearSection = true;
    } else if (light_stopline_distance > 40 &&
               (nolight_stopline_distance > 40) && StopLineDistance > 40 &&
               isNearSection) {
      RunState = "Follow";
      isNearSection = false;
    }
    ...
    /* 控制车速,生成变道路径点 */
    if (RunState == "Start!") {
      SimOneAPI::GenerateRoute(StartAndEnd, indexOfValidPoints,
                               targetpath); // 获得初始的规划路径的点集
    ...
    else if (RunState == "Follow") // 跟车，但是需要改
    {
      if (ObsExist) {
        if (ObsDistance < 40) {
          if (Obstacle_ahead.speed > 0.5 && Obstacle_ahead.speed < 2 &&
              Obstacle_ahead.type == 6) {
            ...
            if (DetectIsTurnable(gpsPtr, Obstacle_ahead, turn, state)) {
              ...
              if (Obstacle_ahead.type == 6) {
                ...
                RunState = "ObstacleAEB";
            }
          }
        }
        /* 根据障碍距离，执行不同动作 */
        if (ObsDistance > 60)
          SlowSpeed.Setpoint(maxspeed);
        else if ((ObsDistance < 60) &&
                 (ObsDistance > mainVehicleSpeed * 1.5 + 15))
          ...
          SlowSpeed.Setpoint(Obstacle_ahead.speed * 0.6);
        if (ObsDistance <= 5)
    ...
    /* 开始变道 */
      ...
      if (turn == "right") { // 左转右转
        ...
        SignalLightsPtr->signalLights =
            ESimOne_Signal_Light_RightBlinker; // 打灯
      } else if (turn == "left") {
      ...
      targetpath = ChangeLanePathWithObstacle(mainVehiclePos, ChangeToLane_ID,
                                              Obstacle_ahead.pt, TurnToPoint);
      StartAndEnd.clear();
      StartAndEnd.push_back(TurnToPoint);
      StartAndEnd.push_back(endPt);
      SimOneAPI::GenerateRoute(StartAndEnd, indexOfValidPoints, Path2);
      ...
      if (mainVehicleSpeed < 1.5) {
        ...
        SlowSpeed.Setpoint(mainVehicleSpeed);
      throttle = SlowSpeed.compute(mainVehicleSpeed);
      if (UtilMath::distance(mainVehiclePos, TurnToPoint) < 3) {
        SignalLightsPtr->signalLights = 0;
        RunState = "Follow";
      }
    }


    else if (RunState == "NearIntersection" ||
               isNearSection) { // 在路口的状态，通过距离路口路径来判断
      std::cout << "(RunState == \"NearIntersection\"||isNearSection)"
                << std::endl;
      if (StopLineDistance < nolight_stopline_distance) {
        std::cout << "(StopLineDistance < nolight_stopline_distance)"
                  << std::endl;
        // 该路口有红绿灯，开始判断是否可以通过；
        ...
        if (IsGreenLight(light.id, currentLaneID, light, mainVehicleSpeed,
                         StopLineDistance) &&
            !CrosswalkOccupied(crosswalk.boundaryKnots, obstaclesPtr) &&
            !IsJunctionCrowed(gpsPtr, naviRoadIdList)) {
          std::cout << "IsGreenLight(light.id, currentLaneID," << std::endl;
          if (ObsDistance > 40)
            SlowSpeed.Setpoint(maxspeed);
          else if ((ObsDistance < 40) &&
                   (ObsDistance > mainVehicleSpeed * 1.5 + 15))
            SlowSpeed.Setpoint(std::min(maxspeed, Obstacle_ahead.speed * 1.1));
          else if ((ObsDistance <= mainVehicleSpeed * 1.5 + 15) &&
                   (ObsDistance > mainVehicleSpeed * 1 + 10))
        ...
        /* 非绿灯或时间不足或有东西在人行道上，停下 */
          ...
          cout << "else" << endl;
          if (ObsExist && Obstacle_ahead.speed >= 3) {
            ...
            else if ((ObsDistance <= 20 ||
                      StopLineDistance <= stop_distance + 15) &&
                     (ObsDistance > 12 || StopLineDistance > stop_distance + 5))
              SlowSpeed.Setpoint(std::min(3.0, Obstacle_ahead.speed));
            else if (ObsDistance <= 12 ||
                     StopLineDistance <= stop_distance + 5) {
              SlowSpeed.Setpoint(-10);
            ...
            if ((StopLineDistance <= stop_distance + 20) &&
              ...
              SlowSpeed.Setpoint(8.0);
                     ...
                     (StopLineDistance > stop_distance))
              SlowSpeed.Setpoint(2.0);
            else if (StopLineDistance <= stop_distance)
              SlowSpeed.Setpoint(-10);
          }
        }
      } else if (StopLineDistance > nolight_stopline_distance) {
        // 没红路灯情况，这一部分有问题，还没发用
        if (!CrosswalkOccupied(crosswalknots, obstaclesPtr) &&
            !IsJunctionCrowed(gpsPtr, naviRoadIdList)) {
          ...
          if (ObsDistance > 60)
            ...
            SlowSpeed.Setpoint(Obstacle_ahead.speed * 0.9);
        } else {
          std::cout << "success" << std::endl;
          if ((std::min(ObsDistance, nolight_stopline_distance + 10) <= 30) &&
              (std::min(ObsDistance, nolight_stopline_distance + 10) > 20))
            SlowSpeed.Setpoint(std::min(7.0, Obstacle_ahead.speed));
          else if ((std::min(ObsDistance, nolight_stopline_distance + 10) <=
                    20) &&
                   (std::min(ObsDistance, nolight_stopline_distance + 10) > 15))
            SlowSpeed.Setpoint(std::min(2.0, Obstacle_ahead.speed));
          else if (std::min(ObsDistance, nolight_stopline_distance + 10) <= 15)
            SlowSpeed.Setpoint(-10);
        }
      }
      throttle = SlowSpeed.compute(mainVehicleSpeed);
      ...
      if (ObsDistance <= 40 && ObsDistance > 30)
        SlowSpeed.Setpoint(std::min(8.0, mainVehicleSpeed));
      else if (ObsDistance <= 30 && ObsDistance > 20)
        SlowSpeed.Setpoint(std::min(3.0, mainVehicleSpeed));
      else if (ObsDistance <= 20)
        SlowSpeed.Setpoint(-10);
      throttle = SlowSpeed.compute(mainVehicleSpeed);
    }
      ...
      if (ObsDistance <= 40 && ObsDistance > 20)
        SlowSpeed.Setpoint(std::min(8.0, mainVehicleSpeed));
      else if (ObsDistance <= 20 && ObsDistance > 10)
        SlowSpeed.Setpoint(std::min(3.0, mainVehicleSpeed));
      else if (ObsDistance <= 10)
        SlowSpeed.Setpoint(-10);
      throttle = SlowSpeed.compute(mainVehicleSpeed);
    }
    ...
    InitialSteering =
        UtilDriver::calculateSteering(targetpath, gpsPtr.get(), forwardIndex);
    ...
    UtilDriver::setDriver(gpsPtr->timestamp, throttle, 0,
                          float(InitialSteering)); // 控制主车
    SimOneAPI::SetSignalLights(MainVehicleId, SignalLightsPtr.get());
    ...
    std::cout << "ObstaclePs" << Obstacle_ahead.pt.x << " "
              << Obstacle_ahead.pt.y << " " << Obstacle_ahead.pt.z << std::endl;
    std::cout << "mainVehicleSpeed==" << mainVehicleSpeed << "m/s" << endl;
    std::cout << "position==" << mainVehiclePos.x << "," << mainVehiclePos.y
              << endl;
    std::cout << "laneID==" << currentLaneID.GetString() << endl;
    std::cout << "light_stopline_distance==" << light_stopline_distance << endl;
    std::cout << "nolight_stopline_distance==" << nolight_stopline_distance
              << endl;
