...
bool GetdetectingZone(const SSD::SimString& currentlaneId, SSD::SimString& ChangeToLine_ID,
...
void GetChangeLanePath(const double& vel, const obstaclestruct& Obstacle, const SSD::SimPoint3D& mainVehiclepos,
    const SSD::SimString& ChangeToLine_ID, SSD::SimPoint3DVector& targetpath) {
    ...
    if (SimOneAPI::GetLaneMiddlePoint(Obstacle.pt, ChangeToLine_ID, changeToPoint,
    ...
    SimOneAPI::GetLaneST(ChangeToLine_ID, changeToPoint, tos, tot);
    SimOneAPI::GetInertialFromLaneST(ChangeToLine_ID, tos + vel * 4, tot, changeToPoint, dir);
...
bool
IsGreenLight(const long& lightId, const SSD::SimString& laneId, const HDMapStandalone::MSignal& light, double speed,
    double distance) {
...
bool CrosswalkOccupied(const SSD::SimPoint3DVector& crosswalk) {
    ...
    for (auto& obstacle : allobstacles) {
        if ((obstacle.type == 4 || obstacle.type == 8) && obstacle.speed != 0) {
...
bool DetectClosestSign(const unique_ptr<SimOne_Data_Gps>& gps, int& warningSignIndex, double& distance) {
    ...
    for (int i = 0; i < TrafficSignList.size(); i++)//通过遍历，找出当前离得最近的交通标志
    {
        ...
        SSD::SimPoint3D lightdis(sign.pt.x, sign.pt.y, sign.pt.z);
        double distanceSign = UtilMath::distance(mainVehiclepos, lightdis);
...
DetectObstacleAhead(const unique_ptr<SimOne_Data_Gps>& gps, obstaclestruct& obstacle, const SSD::SimVector<long>& roadidlist,
    ...
    for (unsigned int i = 0; i < allObstacles.size(); i++) {
        ...
        double distance = UtilMath::distance(SSD::SimPoint3D(MainVehiclePos), obstacle.pt);
        SSD::SimString obstacle_laneid = GetNearMostLane(obstacle.pt);
        HDMapStandalone::MLaneId id(obstacle_laneid);
        if (find(roadidlist.begin(), roadidlist.end(), id.roadId) != roadidlist.end() &&
            (obstacle_laneid != lanelink.leftNeighborLaneName &&
                obstacle_laneid != lanelink.rightNeighborLaneName))//同时在规划路径的road中，同时不是在主lane的左右车道
        {
...
void DetectStopline(HDMapStandalone::MSignal& light, HDMapStandalone::MObject& crosswalk, SSD::SimString& laneId) {
...
bool HavetoTurn(const unique_ptr<SimOne_Data_Gps>& gps, const SSD::SimPoint3DVector& targetpath) {
    //
    return true;
}
...
int main() {
    ...
    while (true) {
        ...
        //        if (currentlaneid != GetNearMostLane(mainVehiclePos)) {
        //            currentlaneid = GetNearMostLane(mainVehiclePos);
        ...
        HDMapStandalone::MSignal light = GetTargetLight(HDMapStandalone::MLaneId(currentlaneid), currentlaneid, naviRoadIdList);
        SSD::SimPoint3D stopline = GetTragetStopLine(light, currentlaneid);
        ...
        //DetectStopline(light, crosswalk, currentlaneid);//获得最近的交通灯关联停车线
        ...
        /* 检测是否在路口 */
        if (UtilMath::distance(mainVehiclePos, stopline) < 30) {
            //(!PassedLight(mainVehiclePos,stopline,currentlaneid))
            RunState = "NearIntersection";
        }
        if (RunState == "NearIntersection" && UtilMath::distance(mainVehiclePos, stopline) > 30) {
            RunState = "Follow";
        }
        ...
        {
            std::cout << "end follow" << std::endl;
            if (ObsExist) {
                if (ObsDistance < 30) {
                    if (Obstacle_ahead.speed < 3 && Obstacle_ahead.type == 6 && DetectIsTurnable(gpsPtr, Obstacle_ahead, turn, state)) {
                        RunState = "ChangeLaneStart";//如果是车则变为变道
                    }
                    else if (Obstacle_ahead.speed <= 0.5 && (Obstacle_ahead.type != 4 && Obstacle_ahead.type != 6 && Obstacle_ahead.type != 8) && DetectIsTurnable(gpsPtr, Obstacle_ahead, turn, state)) {
                        RunState = "ObstacleAvoid";//如果是障碍则开启避障
                    }
                }
                /* 根据障碍距离，执行不同动作 */
        ...
        /* 开始变道 */
            ...
            if (turn == "right") {//左转右转
                ChangeToLine_ID = lanelink.rightNeighborLaneName;
                cout << "ChangeToLine_ID" << ChangeToLine_ID.GetString() << endl;
                pSignalLights->signalLights = ESimOne_Signal_Light_RightBlinker;//打灯
            }
            else if (turn == "left") {
                ChangeToLine_ID = lanelink.leftNeighborLaneName;
                cout << "ChangeToLine_ID" << ChangeToLine_ID.GetString() << endl;
                pSignalLights->signalLights = ESimOne_Signal_Light_LeftBlinker;
            }
            targetpath.clear();//构建path
            SSD::SimPoint3DVector Path1, Path2;
            Path1 = ChangeLanePathWithLane(mainVehiclePos, mainVehicleSpeed, ChangeToLine_ID, TurnToPoint);
            ...
            targetpath = ChangeLanePathWithObstacle(mainVehiclePos, ChangeToLine_ID, Obstacle_ahead.pt, TurnToPoint);
            StartAndEnd.clear();
            StartAndEnd.push_back(TurnToPoint);
            StartAndEnd.push_back(endPt);
            SimOneAPI::GenerateRoute(StartAndEnd, indexOfValidPoints, Path2);
        ...
        else if (RunState == "NearIntersection") {//在路口的状态，通过距离路口路径来判断
            if (1) {
                //该路口有红绿灯，开始判断是否可以通过；
                //绿灯时间充足或者没有灯，获取前驱道路信息后继续开
                //停下后就开始等红灯了，等完开始继续跟车知道过了路口
                if (IsGreenLight(light.id, currentlaneid, light, mainVehicleSpeed, light_stopline_distance) &&
                    ...
                    else if (ObsDistance <= mainVehicleSpeed * 1 + 10) SlowSpeed.Setpoint(Obstacle_ahead.speed * 0.5);
                    if (ObsDistance <= 5)  SlowSpeed.Setpoint(-10);
                ...
                /* 非绿灯或时间不足或有东西在人行道上，停下 */
                    ...
                    if (ObsExist) {
                        if ((ObsDistance <= 30 || StopLineDistance <= stop_distance + 20) &&
                            (ObsDistance > 20 || StopLineDistance > stop_distance + 10))
                            SlowSpeed.Setpoint(std::min(8.0, Obstacle_ahead.speed * 1.1));
                        else if ((ObsDistance <= 20 || StopLineDistance <= stop_distance + 10) &&
                            (ObsDistance > 12 || StopLineDistance > stop_distance))
                            SlowSpeed.Setpoint(std::min(3.0, Obstacle_ahead.speed));
                        else if (ObsDistance <= 12 || StopLineDistance <= stop_distance) {
                        ...
                        else if (StopLineDistance <= stop_distance)SlowSpeed.Setpoint(-10);
                    }
                }
            }
            else if (light_stopline_distance > nolight_stopline_distance) {//没红路灯情况，这一部分有问题，还没发用
                if (!CrosswalkOccupied(crosswalk.boundaryKnots) && !IsJunctionCrowed(gpsPtr, naviRoadIdList)) {
                    if (ObsDistance > 60) SlowSpeed.Setpoint(maxspeed);
                    else if ((ObsDistance < 60) && (ObsDistance > mainVehicleSpeed * 1.5 + 15))
                        SlowSpeed.Setpoint(std::min(maxspeed, Obstacle_ahead.speed * 1.1));
                    else if ((ObsDistance <= mainVehicleSpeed * 1.5 + 15) &&
                    ...
                    if (Passedstopline(mainVehiclePos, nolight_stopline, currentlaneid)) {
                        ...
                        if ((ObsDistance <= 30 || nolight_stopline_distance <= stop_distance + 20) &&
                            (ObsDistance > 20 || nolight_stopline_distance > stop_distance + 10))
                            SlowSpeed.Setpoint(std::min(8.0, Obstacle_ahead.speed * 1.1));
                        else if ((ObsDistance <= 20 || nolight_stopline_distance <= stop_distance + 10) &&
                            (ObsDistance > 12 || nolight_stopline_distance > stop_distance + 3))
                            SlowSpeed.Setpoint(std::min(3.0, Obstacle_ahead.speed));
                        else if (ObsDistance <= 12 || nolight_stopline_distance <= stop_distance + 3)
                            SlowSpeed.Setpoint(0);
                    }
                }
            }
            throttle = SlowSpeed.compute(mainVehicleSpeed);
        ...
        if (!targetpath.empty()) {//判断是否有路近点，防止程序崩了
            InitialSteering = UtilDriver::calculateSteering(targetpath, gpsPtr.get());
        }
        else {
            SimOneAPI::GenerateRoute(StartAndEnd, indexOfValidPoints, targetpath);//获得初始的规划路径的点集
            PrintTargetPath(targetpath);
